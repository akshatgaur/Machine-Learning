...
def pooling_layer_forward(input, layer):
  ...
  for batch in range(batch_size):
    input_n = dict(input)
    input_n['data'] = input_n['data'][:,batch]
    col = im2col_conv(input_n=input_n, layer=layer, h_out=h_out, w_out=w_out)
    col = col.reshape(k*k, c*h_out*w_out)
    # output['data'][: , batch] = col.max(axis=0).flatten()
    output['data'][:, batch] = col.max(axis=0)
    # chunk = len(col) / c
    # for i in range(c):
    #   for j in range(len(col[0])):
    #     val = max(col[i * chunk : i * chunk + chunk, j])
    #     output['data'][i * len(col[0]) + j, batch] = val
...
def pooling_layer_backward(output, input, layer):
  ...
  for batch in range(batch_size):
    input_n = dict(input)
    input_n['data'] = input_n['data'][:,batch]
    col = im2col_conv(input_n=input_n, layer=layer, h_out=h_out, w_out=w_out).reshape(k*k, c*h_out*w_out)
    out_col = np.zeros((k * k, c * h_out * w_out))

    idx = np.argmax(col, axis=0)
    tmp_data_diff = output['diff'][:, batch]
    out_col[idx, range(idx.size)] = tmp_data_diff
    out_col = out_col.reshape(k * k * c, h_out * w_out)

    # chunk = len(col) / c
    # k = 0
    # for i in range(c):
    #   for j in range(len(col[0])):
    #     val = np.argmax(col[i * chunk : i * chunk + chunk, j])
    #     out_col[val,j] = output['diff'][k, batch]
    #     k += 1

    out = col2im_conv(out_col,input=input,layer=layer,h_out=h_out,w_out=w_out)
    input_od[:, batch] = out.flatten()
...
def relu_forward(input, layer):
  ...
  # implementation begins
  output['data'] = np.array(input['data'])
...
def relu_backward(output, input, layer):
  ...
  # implementation begins
  input_od[input['data'] > 0.0] = output['diff'][input['data'] > 0.0]
  # implementation ends
...
def inner_product_forward(input, layer, param):
  ...
  for batch in range(batch_size):
    output['data'][:, batch] = np.dot(param['w'].T,input['data'][:,batch]) + param['b']
...
def sgd_momentum(w_rate, b_rate, mu, decay, params, param_winc, param_grad):
  ...
  #  iterate over all layers except for the data layer
  for layer in params_:

    param = params_[layer]
    # extract theta from param_winc_
    thetas = param_winc_[layer]

    # extract gradient dw and db from param
    gradient = param_grad[layer]

    # update gradient dw with weight decay (formula 13)
    dw = gradient['w'] + decay * param['w']

    # update theta with formula (9)
    thetas['w'] = mu * thetas['w'] + w_rate * dw
    thetas['b'] = mu * thetas['b'] + w_rate * gradient['b']

    # update w and b with formula (10)
    param['w'] -= thetas['w']
    param['b'] -= thetas['b']
